...
class World(object):
    def __init__(self, carla_world, client, hud, args):
        ...
        try:
            ...
            sys.exit(1)
        self.safetypotential = SafetyPotential(self.world, self.map)
        self.hud = hud
        self.player = None
    ...
    def restart(self):
        ...
        self.safetypotential.Assign_Player(self.player)
        spawn_points_sorted = sorted(spawn_points, key=lambda pt : pt.location.distance(spawn_point.location))
        ...
        spawn_points_sorted = sorted(spawn_points, key=lambda pt : pt.location.distance(spawn_point.location))
        for transform in spawn_points_sorted:
            if len(batch) > 20:
                break
        ...
        all_vehicle_actors = self.world.get_actors(self.npc_vehicle_list)
        for actor in all_vehicle_actors:
            traffic_manager.ignore_lights_percentage(actor,100.0)
            #traffic_manager.ignore_vehicles_percentage(actor, 50.0)
        traffic_manager.ignore_lights_percentage(self.player,100.0)
        self.safetypotential.Assign_NPCS(all_vehicle_actors)
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        #print("world")
        #vehicles = [x for x in actors if x.type_id.startswith("vehicle")]
        #print(vehicles)
        #print("location : ", vehicles[1].get_location())
        #print("velocity : ", vehicles[1].get_velocity())
        #print("control : ", vehicles[1].get_control())
...
def game_loop(args):
    ...
    try:
        ...
        world = World(sim_world, client, hud, args)
